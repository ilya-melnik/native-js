...
// ?
// // let bestFirst = students.map(st => (st.name))
// let sortByScore = students.map(st => st.name).sort()
...
// }
 ...
 const sortByName = students.sort( (a,b)=> a.name > b.name? 1: -1 )
...
// console.log(sortedByName);
 ...
 let qqq = students.sort( (a, b)=> b.scores - a.scores)
...
let topStudents = deepCopyStudents.sort((a, b) => b.scores - a.scores)
...
let studentsNames = students.map(stName => stName.name)
...
let nameWithSpace = students.map(stName => stName.name).join(" ")
let namesWithComma = students.map(stName => stName.name).join(', ')
...
console.log(students.find(st => st.name === 'Ann'))
...
// console.log(students.sort( (a,b)=> b.scores - a.scores)[0])
...
// let grads = students.sort( (a,b)=> b.scores - a.scores)
// console.log(grads)
// console.log(grads[0])
// console.log(grads[1])
...
//
// // 14.Напишите функцию addFriends, которая принимает параметром массив students и возвращает новый массив, при этом добавляет k каждому студенту свойство .friends, значением которого является массив имён всех остальных студентов из массива, за исключением собственного имени студента. Т.е. в друзьях у Боба Боба быть не должно.
